...
export default function Today(){
 ...
 return <AppShell>
  ...
  <div className="mb-4 flex flex-wrap items-center justify-between gap-2"><h2 className="text-xl font-bold">Da fare oggi</h2><Link href="/calendario" className="text-sm font-bold text-sage-700">Vedi calendario</Link></div>
...
function AppointmentCard({appointment,patients,sessions,goals}:{appointment:Appointment;patients:Patient[];sessions:ReturnType<typeof useData>["data"]["sessions"];goals:ReturnType<typeof useData>["data"]["goals"]}){
 ...
 return <article className="card p-4 sm:p-5"><div className="flex flex-wrap gap-3 sm:flex-nowrap sm:gap-4"><time className="w-12 shrink-0 pt-1 text-sm font-bold text-sage-700">{appointment.time}</time><div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-sage-100 font-bold">{initials(patient)}</div><div className="min-w-0 basis-full flex-1 sm:basis-auto"><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold">{fullName(patient)}</h3><span className="text-sm text-slate-500">{age(patient.birthDate)||"—"} anni</span></div><p className="mt-1 break-words text-sm font-medium text-sage-700">{goal?.title||patient.referralReason}</p><p className="mt-2 break-words text-sm text-slate-500">{last?`Ultima seduta: ${last.result||last.activities}`:"Nessuna seduta precedente"}</p>{appointment.notes&&<p className="mt-2 break-words rounded-lg bg-[#fff8ed] px-3 py-2 text-sm text-[#8b6843]">Da ricordare: {appointment.notes}</p>}<div className="mt-4 grid gap-2 sm:flex"><Link href={`/pazienti/${patient.id}`} className="btn btn-quiet text-sm">Apri paziente</Link><Link href={`/sedute/nuova?a=${appointment.id}`} className="btn btn-primary text-sm">Inizia seduta</Link></div></div></div></article>
}
