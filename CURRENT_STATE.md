...
- Il nuovo codice della Libreria terapeutica V2 è pubblicato e in collaudo manuale nello stato intermedio con `014` applicata e `015` non applicata. Le vecchie policy Storage restano attive; solo dopo il collaudo potrà essere valutata la `015`. Il collaudo ha rilevato submit upload ripetibili e una cancellazione il cui errore veniva propagato nel vecchio stato globale di sincronizzazione; la correzione locale rende upload/delete single-flight, usa errori Materiali specifici e mantiene delete/quota idempotenti, ma non è ancora pubblicata. L'audit di reconciliation è report-only, pagina l'intero dataset DB e percorre ricorsivamente il namespace Storage. Le reservation scadute o con cleanup fallito mantengono i byte prenotati.
