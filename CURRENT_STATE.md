...
- Il nuovo codice della Libreria terapeutica V2 è pubblicato e il collaudo manuale è ancora in corso nello stato intermedio con `014` e `016` applicate e `015` non applicata. Upload, quota e DELETE produzione sono operativi; il postflight della `016` ha confermato i soli grant server previsti e nessun nuovo privilegio DML su `session_materials`. Le vecchie policy Storage restano attive; solo dopo il collaudo potrà essere valutata la `015`. L'audit di reconciliation è report-only, pagina l'intero dataset DB e percorre ricorsivamente il namespace Storage. Le reservation scadute o con cleanup fallito mantengono i byte prenotati.
- La migration `017` è presente in produzione, ma il nuovo codice atomico per metadati/link/associazioni e le correzioni di apertura link, errori separati per modalità e preview immagini non sono ancora stati pubblicati. Il prossimo passo è un deploy controllato seguito dal collaudo dei bug reali; la `015` resta non applicata.
