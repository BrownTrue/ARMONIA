...
- Sono presenti le fondamenta applicative della Clinical Assessment V2: union discriminata tramite `schemaVersion`, payload modulare, registry dei moduli e dispatch interno V1/V2. In modalità locale “Nuova valutazione” apre il flusso modulare con scelta di tipo, data clinica e aree, quindi il wizard dal primo passaggio “Motivo dell’accesso”. Tutti i sei passaggi sono compilabili sullo stesso draft. L’Anamnesi usa categorie attivabili singolarmente, così i campi non pertinenti restano chiusi e assenti dal payload. `language_oral` V1 resta il primo e unico modulo disponibile. Autosave debounced, coda FIFO e “Salva e chiudi” operano sull’intero draft. La valutazione V2 locale può essere completata se ha data clinica e almeno un modulo, diventa sola lettura e può essere modificata soltanto tramite correzione esplicita senza autosave. È disponibile la stampa A4 V2 con branding condiviso, dispatch separato dalla V1, sezioni comuni e moduli ricavati dagli adapter del registry; gli obiettivi correnti del percorso vengono letti come entità esterne e non duplicati nel payload. La V1 storica continua a usare il proprio wizard e renderer. La V2 è intenzionalmente disponibile e scrivibile soltanto in modalità locale: in cloud resta attivo il precedente flusso V1 e tutte le scritture V2 sono bloccate applicativamente. Nessuna migration V2 è stata creata o applicata e la persistenza Supabase V2 resta disabilitata in attesa della decisione su `module_type`.
