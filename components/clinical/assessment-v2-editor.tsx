...
function ClinicalModuleSelector({ draft, onToggle, onClose }: { draft: ClinicalAssessmentV2; onToggle: (definition: (typeof clinicalModuleRegistry)[number]) => void; onClose: () => void }) {
  return <Modal title="Aree cliniche" onClose={onClose}><p className="text-sm leading-6 text-slate-500">Apri una macro-area e scegli i moduli pertinenti. Le macro-aree servono soltanto a organizzare la selezione.</p><div className="mt-5 space-y-3">{clinicalModuleGroups.map((group, index) => <details key={group.code} open={index === 0} className="overflow-hidden rounded-2xl border border-sage-100 bg-white"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-4 font-bold"><span>{group.label}</span><span className="text-sage-700">⌄</span></summary><div className="space-y-2 border-t border-sage-100 bg-sage-50/30 p-3">{getClinicalModulesForGroup(group).map((definition) => { const present = draft.data.modules.some((module) => module.code === definition.code && module.version === definition.version); return <button type="button" key={`${definition.code}@${definition.version}`} aria-pressed={present} onClick={() => onToggle(definition)} className={`flex w-full items-center justify-between gap-3 rounded-xl border px-4 py-3 text-left text-sm ${present ? "border-sage-400 bg-sage-100 font-bold text-sage-800" : "border-sage-100 bg-white text-slate-700 hover:border-sage-300"}`}><span>{definition.label}</span><span className="shrink-0 text-xs font-bold">{present ? "✓ Aggiunta · Rimuovi" : "+ Aggiungi"}</span></button>; })}</div></details>)}</div><div className="mt-6 flex justify-end"><button type="button" onClick={onClose} className="btn btn-primary">Fatto</button></div></Modal>;
}
