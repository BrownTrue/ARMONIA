...
export function AssessmentV2Editor({ source }: { source: ClinicalAssessmentV2 }) {
  ...
  return <AppShell>{readOnly && <ClinicalAssessmentPrint patientName={fullName(patient)} assessment={draft} pathwayTitle={pathway?.title} professional={data.profile} logoSrc={logoSrc} goals={pathwayGoals} />}<div className="assessment-screen-only mx-auto max-w-5xl">
    ...
    <header className="mt-5 flex flex-col items-start gap-4 sm:flex-row sm:flex-wrap sm:justify-between"><div className="min-w-0"><p className="break-words text-sm font-semibold text-sage-700">{fullName(patient)}</p><h1 className="mt-1 break-words text-2xl font-bold sm:text-3xl">{clinicalAssessmentTypeLabel(draft.assessmentType)}</h1><p className="mt-2 text-sm text-slate-500">Data clinica {draft.clinicalDate ? new Date(`${draft.clinicalDate}T12:00:00`).toLocaleDateString("it-IT") : "non indicata"}</p></div><div className="sm:text-right"><span className="chip">{correcting ? "Correzione in corso" : readOnly ? "Completata" : "Bozza locale"}</span><p className={`mt-2 text-xs ${saveState === "error" ? "text-red-600" : "text-slate-400"}`}>{correcting ? (dirty ? "Modifiche non salvate · autosave disattivato" : "Salvataggio manuale · autosave disattivato") : readOnly ? "Sola lettura" : saveState === "saving" ? "Salvataggio…" : saveState === "error" ? "Errore di salvataggio" : "Salvato"}</p></div></header>
...
function DeleteV2AssessmentModal({ assessment, onClose, onConfirm }: { assessment: ClinicalAssessmentV2; onClose: () => void; onConfirm: () => Promise<void> }) {
  ...
  const confirm = async () => { setDeleting(true); try { await onConfirm(); } finally { setDeleting(false); } };
  return <Modal title={completed ? "Elimina valutazione completata" : "Elimina bozza"} onClose={onClose}><div className="space-y-4 text-sm leading-6 text-slate-600"><p><strong>{clinicalAssessmentTypeLabel(assessment.assessmentType)}</strong><br/>Data clinica: {date}</p><p>{completed ? "Vuoi eliminare definitivamente questa valutazione? Questa operazione non può essere annullata." : "Vuoi eliminare questa bozza?"} Verrà eliminata esclusivamente la valutazione selezionata; percorso clinico, paziente, sedute e obiettivi resteranno invariati.</p>{completed && <label className="block font-bold text-ink">Digita ELIMINA per confermare<input autoComplete="off" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} className="mt-2 w-full rounded-xl border border-sage-100 bg-white px-3 py-2.5 font-normal outline-none focus:border-sage-500" /></label>}</div><div className="form-actions mt-6"><button type="button" onClick={onClose} className="btn btn-quiet">Annulla</button><button type="button" disabled={deleting || (completed && confirmation !== "ELIMINA")} onClick={() => void confirm()} className="btn bg-red-600 text-white disabled:cursor-not-allowed disabled:opacity-40">{deleting ? "Eliminazione…" : completed ? "Elimina definitivamente" : "Elimina bozza"}</button></div></Modal>;
}
