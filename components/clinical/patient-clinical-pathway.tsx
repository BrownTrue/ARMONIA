...
function AssessmentPanel({ assessments, onStart, patientId }: { pathway: ClinicalPathway; assessments: ReturnType<typeof useData>["data"]["clinicalAssessments"]; onStart: () => void; patientId: string }) {
  ...
  const areaLabel = (assessment: (typeof assessments)[number]) => assessment.schemaVersion === 1 ? "Linguaggio e comunicazione" : assessment.data.modules.map((module) => clinicalModuleRegistry.find((item) => item.code === module.code && item.version === module.version)?.label).filter(Boolean).join(" · ") || "Aree cliniche da selezionare";
