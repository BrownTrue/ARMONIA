...
import type {AppData,Appointment,Goal,Material,Patient,Profile,Session} from "../types.ts";
...
export async function loadCloudData(client:SupabaseClient,user:User):Promise<AppData>{
  const [profiles,patients,appointments,sessions,goals,materials,patientMaterials,sessionMaterials,sessionGoals,clinical]=await Promise.all([
  ...
  return {
    ...
    appointments:(appointments.data||[]).map((a):Appointment=>{const d=splitDate(a.starts_at);return {id:a.id,patientId:a.patient_id,date:d.date,time:d.time,duration:a.duration_minutes,type:a.type,notes:a.notes||"",recurrenceSeriesId:a.recurrence_series_id||undefined,createdAt:a.created_at}}),
    sessions:(sessions.data||[]).map((s):Session=>({id:s.id,patientId:s.patient_id,appointmentId:s.appointment_id||undefined,date:splitDate(s.occurred_at).date,duration:s.duration_minutes||45,goalIds:(sessionGoals.data||[]).filter(x=>x.session_id===s.id).map(x=>x.goal_id),activities:s.activities||"",response:s.patient_response||"",helpLevel:s.help_level||"",result:s.result||"",nextPlan:s.next_session_plan||"",homework:s.homework||"",notes:s.notes||"",materialIds:(sessionMaterials.data||[]).filter(x=>x.session_id===s.id).map(x=>x.material_id),createdAt:s.created_at})),
...
export const appointmentRow=(a:Appointment,userId:string)=>({id:a.id,user_id:userId,patient_id:a.patientId,starts_at:joinDate(a.date,a.time),duration_minutes:a.duration,type:a.type,notes:a.notes||null,recurrence_series_id:a.recurrenceSeriesId||null,updated_at:new Date().toISOString()});
export const sessionRow=(s:Session,userId:string)=>({id:s.id,user_id:userId,patient_id:s.patientId,appointment_id:s.appointmentId||null,occurred_at:joinDate(s.date,"12:00"),duration_minutes:s.duration,activities:s.activities||null,patient_response:s.response||null,help_level:s.helpLevel||null,result:s.result||null,next_session_plan:s.nextPlan||null,homework:s.homework||null,notes:s.notes||null,updated_at:new Date().toISOString()});
