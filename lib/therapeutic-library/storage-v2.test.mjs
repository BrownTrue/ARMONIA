...
import { deleteTherapeuticMaterial, materialDeleteErrorMessage, materialSaveErrorMessage, saveTherapeuticMaterialMetadata } from "./material-api.ts";
...
const migration016 = await readFile(new URL("../../supabase/migrations/016_therapeutic_library_server_grants.sql", import.meta.url), "utf8");
const migration017 = await readFile(new URL("../../supabase/migrations/017_therapeutic_library_atomic_metadata.sql", import.meta.url), "utf8");
...
test("client non può scrivere campi storage-authoritative dopo enforcement", () => {
  ...
  assert.match(saveRoute, /rpc\("save_therapeutic_material_metadata"/);
  assert.doesNotMatch(saveRoute, /body\.(?:storagePath|storage_path|fileName|file_name|mimeType|mime_type|size|file_size)/);
  assert.doesNotMatch(saveRoute, /from\("materials"\)\.(?:insert|update)/);
  assert.match(migration017, /null, null, null, 0/);
});

test("016 concede al server soltanto i privilegi diretti richiesti dalla Libreria V2", () => {
  assert.match(migration016, /grant select, insert, update on table public\.materials to service_role;/i);
  assert.match(migration016, /grant insert, delete on table public\.patient_materials to service_role;/i);
  assert.match(migration016, /grant select on table public\.patients to service_role;/i);
  assert.doesNotMatch(migration016, /grant all|truncate|trigger|references/i);
  assert.doesNotMatch(migration016, /to (?:anon|authenticated)/i);
  assert.doesNotMatch(migration016, /session_materials/i);
  assert.doesNotMatch(migration016, /grant delete on table public\.materials/i);
});

test("017 sostituisce le mutazioni dirette del save senza revocare i grant 016", () => {
  assert.match(saveRoute, /rpc\("save_therapeutic_material_metadata"/);
  assert.doesNotMatch(saveRoute, /from\("(?:materials|patient_materials|patients)"\)/);
  assert.match(deleteRoute, /from\("materials"\)\.select/);
  assert.match(deleteRoute, /rpc\("delete_therapeutic_material_record"/);
  assert.match(migration016, /grant select, insert, update on table public\.materials to service_role/i);
  assert.doesNotMatch(migration017, /revoke .* from service_role/i);
  assert.match(migration015, /revoke all privileges on table public\.materials from authenticated/);
  assert.doesNotMatch(migration015, /from service_role/);
});
...
test("link e associazioni sono salvati con una sola RPC transazionale e idempotente per ID", () => {
  assert.match(saveRoute, /rpc\("save_therapeutic_material_metadata"/);
  assert.doesNotMatch(saveRoute, /from\("patient_materials"\)/);
  assert.doesNotMatch(saveRoute, /from\("materials"\)\.(?:insert|update)/);
  assert.match(saveRoute, /p_material_id: requestedId/);
  assert.match(migration017, /select \*[\s\S]*where id = p_material_id[\s\S]*and user_id = p_user_id[\s\S]*for update/);
  assert.match(migration017, /if found then[\s\S]*update public\.materials[\s\S]*else[\s\S]*insert into public\.materials/);
  assert.equal((migration017.match(/insert into public\.materials/g) || []).length, 1);
});

test("link resta quota-free e non possiede campi Storage", () => {
  assert.match(migration017, /file_name,[\s\S]*storage_path,[\s\S]*mime_type,[\s\S]*file_size,[\s\S]*external_url/);
  assert.match(migration017, /array\[\]::text\[\]\), null, null, null, 0,[\s\S]*btrim\(p_external_url\)/);
  assert.doesNotMatch(migration017, /user_storage_accounts|used_bytes|reserved_bytes|storage_upload_reservations/);
});

test("replace associazioni tratta lista vuota come rimozione completa ed è atomico", () => {
  assert.match(migration017, /delete from public\.patient_materials where material_id = p_material_id/);
  assert.match(migration017, /unnest\(coalesce\(p_patient_ids, array\[\]::uuid\[\]\)\)/);
  assert.ok(migration017.indexOf("delete from public.patient_materials") < migration017.indexOf("insert into public.patient_materials"));
  assert.match(migration017, /where user_id = p_user_id[\s\S]*id = any/);
});

test("017 limita la funzione al service role e isola ogni utente", () => {
  assert.match(migration017, /security definer[\s\S]*set search_path = pg_catalog, public/);
  assert.match(migration017, /revoke all on function[\s\S]*from public, anon, authenticated/);
  assert.match(migration017, /grant execute on function[\s\S]*to service_role/);
  assert.doesNotMatch(migration017, /grant .* to authenticated/);
});

test("client metadata distingue URL, sessione, rete e server senza MIME file", async () => {
  await assert.rejects(() => saveTherapeuticMaterialMetadata({}, async () => new Response(JSON.stringify({ error: "invalid_external_url" }), { status: 400 })), error => materialSaveErrorMessage(error, "link").includes("HTTP o HTTPS"));
  await assert.rejects(() => saveTherapeuticMaterialMetadata({}, async () => new Response(JSON.stringify({ error: "material_auth_required" }), { status: 401 })), error => materialSaveErrorMessage(error, "link").includes("sessione è scaduta"));
  await assert.rejects(() => saveTherapeuticMaterialMetadata({}, async () => { throw new Error("offline"); }), error => materialSaveErrorMessage(error, "link").includes("raggiungere il server"));
  await assert.rejects(() => saveTherapeuticMaterialMetadata({}, async () => new Response(JSON.stringify({ error: "material_save_failed" }), { status: 500 })), error => materialSaveErrorMessage(error, "link") === "Non è stato possibile salvare il link. Riprova.");
  assert.doesNotMatch(materialSaveErrorMessage(new Error("failure"), "link"), /formato/i);
});

test("apertura link evita viewer e download mentre le immagini usano contain", () => {
  assert.match(materialsPage, /m\.externalUrl \? void openMaterial\(m\) : setPreview\(m\)/);
  assert.match(materialsPage, /Apri link ↗/);
  assert.match(dataProvider, /window\.open\(m\.externalUrl,"_blank","noopener,noreferrer"\)/);
  assert.match(materialsPage, /<img[\s\S]*max-h-\[60vh\] max-w-full object-contain/);
  assert.match(materialsPage, /:pdf\?<iframe/);
});

test("cambio modalità pulisce file ed errori specifici", () => {
  assert.match(materialsPage, /setMode\("link"\);setFile\(undefined\);setFileError\(undefined\)/);
  assert.match(materialsPage, /setMode\("file"\);setFileError\(undefined\);setV\(old=>\(\{\.\.\.old,externalUrl:""\}\)\)/);
  assert.match(materialsPage, /mode === "file" && Boolean\(file\) \? materialUploadErrorMessage/);
  assert.match(materialsPage, /materialSaveErrorMessage\(cause, mode === "link" \? "link" : "material"\)/);
});
