...
test("grant 016 coprono le query dirette senza duplicare le SECURITY DEFINER", () => {
  assert.match(saveRoute, /from\("materials"\)\.select/);
  assert.match(saveRoute, /from\("materials"\)\.update/);
  assert.match(saveRoute, /from\("materials"\)\.insert/);
  assert.match(saveRoute, /from\("patient_materials"\)\.delete/);
  assert.match(saveRoute, /from\("patient_materials"\)\.insert/);
  assert.match(saveRoute, /from\("patients"\)\.select/);
