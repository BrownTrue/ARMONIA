...
import { createSingleFlightRunner, isGoogleDeleteAlreadyAbsent, processQueueSnapshot, reconcileStaleUpserts } from "./sync-queue.ts";
import { googleOAuthResponseError } from "./oauth-error.ts";
import { reconnectGoogleConnection } from "./reconnect.ts";
import { createGoogleOAuthResultConsumer } from "./oauth-result.ts";
import { withGoogleFlushLock } from "./queue-lock.ts";
import { resyncAllGoogleAppointments, syncPendingGoogleOperations } from "./settings-actions.ts";
import { googleSyncErrorPresentation, googleSyncStatusPresentation } from "./user-facing-status.ts";

const settingsPage = await readFile(new URL("../../app/impostazioni/page.tsx", import.meta.url), "utf8");

test("la sezione Google è un accordion accessibile e responsive", () => {
  assert.match(settingsPage, /aria-expanded=\{googleExpanded\}/);
  assert.match(settingsPage, /aria-controls="google-calendar-settings"/);
  assert.match(settingsPage, /<button type="button"[^>]*aria-expanded/);
  assert.match(settingsPage, /focus-visible:ring-2/);
  assert.match(settingsPage, /syncState\.pending>0/);
});

test("l'accordion apre automaticamente gli stati problematici ma non quello sano", () => {
  assert.match(settingsPage, /syncPresentation\.kind!=="active"/);
  assert.match(settingsPage, /setGoogleExpanded\(true\)/);
  assert.match(settingsPage, /googleAutoOpened\.current=true/);
});

test("gli errori OAuth vengono presentati come richiesta di ricollegamento", () => {
  assert.deepEqual(
    googleSyncErrorPresentation("87 modifiche non sincronizzate. Rinnovo autorizzazione Google non riuscito: invalid_grant"),
    { message: "Google Calendar deve essere ricollegato.", reconnectRequired: true },
  );
});

test("gli errori temporanei non espongono dettagli tecnici", () => {
  const presentation = googleSyncErrorPresentation("Google Calendar (503): backendError con dettagli tecnici");
  assert.deepEqual(presentation, {
    message: "Alcuni appuntamenti non sono ancora stati sincronizzati. Riprova tra poco.",
    reconnectRequired: false,
  });
  assert.equal(presentation.message.includes("503"), false);
  assert.equal(googleSyncErrorPresentation(), null);
});

test("la riconnessione richiesta prevale su collegamento e pending", () => {
  assert.deepEqual(googleSyncStatusPresentation("invalid_grant", 71), {
    kind: "reconnect",
...
test("pending temporanei e stato sano sono mutuamente esclusivi", () => {
  assert.deepEqual(googleSyncStatusPresentation("errore temporaneo", 71), {
    kind: "pending",
    title: "Sincronizzazione in attesa",
    message: "71 modifiche non sono ancora state sincronizzate.",
  });
  assert.equal(googleSyncStatusPresentation(undefined, 0).kind, "active");
});

test("Sincronizza ora elabora la coda senza riaccodare tutti gli appointment", async () => {
  let flushes = 0;
  let queueAllCalls = 0;
  await syncPendingGoogleOperations(async () => { flushes += 1; });
  assert.equal(flushes, 1);
  assert.equal(queueAllCalls, 0);
});

test("la risincronizzazione completa usa l'accodamento totale separato", async () => {
  let queueAllCalls = 0;
  await resyncAllGoogleAppointments(async () => { queueAllCalls += 1; });
  assert.equal(queueAllCalls, 1);
});

test("google=connected viene consumato una sola volta", () => {
  const consume = createGoogleOAuthResultConsumer();
  assert.equal(consume("connected"), "connected");
  assert.equal(consume("connected"), null);
});

test("google=reconnected viene consumato una sola volta", () => {
  const consume = createGoogleOAuthResultConsumer();
  assert.equal(consume("reconnected"), "reconnected");
  assert.equal(consume("reconnected"), null);
});

test("due flush cross-tab non entrano contemporaneamente nella sezione critica", async () => {
  let tail = Promise.resolve();
  const locks = { request(_name, _options, callback) { const next = tail.then(callback); tail = next.catch(() => {}); return next; } };
  let active = 0;
  let maximum = 0;
  const work = () => withGoogleFlushLock(async () => {
    active += 1;
...
test("un elemento accodato durante un passaggio viene processato nel passaggio successivo", async () => {
  const queue = ["first"];
  const processed = [];
  let runner;
  runner = createSingleFlightRunner(async () => {
...
test("elementi riusciti non ricompaiono, errori temporanei restano e la coda può arrivare a zero", async () => {
  const queue = [{ id: "success" }, { id: "temporary" }];
  await processQueueSnapshot([...queue], id => queue.find(item => item.id === id), async item => {
    if (item.id === "temporary") throw new Error("temporaneo");
    queue.splice(queue.indexOf(item), 1);
  });
  assert.deepEqual(queue.map(item => item.id), ["temporary"]);
  await processQueueSnapshot([...queue], id => queue.find(item => item.id === id), async item => { queue.splice(queue.indexOf(item), 1); });
  assert.deepEqual(queue, []);
});

test("la coda legacy esistente resta leggibile senza migrazione", () => {
  const legacy = '[{"id":"q1","key":"appointment-1","action":"delete","appointmentId":"appointment-1"}]';
  assert.deepEqual(JSON.parse(legacy), [{ id: "q1", key: "appointment-1", action: "delete", appointmentId: "appointment-1" }]);
});
