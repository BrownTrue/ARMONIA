...
import {createSingleFlightRunner,processQueueSnapshot,reconcileStaleUpserts} from "@/lib/google-calendar/sync-queue";
import {withGoogleFlushLock,withGoogleQueueMutationLock} from "@/lib/google-calendar/queue-lock";
...
export function getGoogleSyncState():GoogleSyncState{const state=read<Omit<GoogleSyncState,"pending"|"syncing">>(STATE_KEY,{});return {...state,pending:read<Operation[]>(QUEUE_KEY,[]).length,syncing:flushRunner.running}}
...
export function clearGoogleSyncError(){const state=read<Omit<GoogleSyncState,"pending"|"syncing">>(STATE_KEY,{});delete state.error;write(STATE_KEY,state);announce()}
...
async function mutateQueue(mutation:()=>void){await withGoogleQueueMutationLock(async()=>{mutation()});void flushGoogleCalendarQueue()}

export function queueGoogleUpsert(appointment:Appointment,patient:Pick<Patient,"firstName"|"lastName">){if(!getGoogleCalendarPreferences().enabled)return;void mutateQueue(()=>replaceOperation({key:appointment.id,action:"upsert",appointment,patient}))}
export function queueGoogleDelete(appointmentId:string){if(!getGoogleCalendarPreferences().enabled)return;void mutateQueue(()=>{const mapped=Boolean(read<Record<string,string>>(MAPPING_KEY,{})[appointmentId]),queued=read<Operation[]>(QUEUE_KEY,[]).some(item=>item.key===appointmentId);if(!cloudMode&&!mapped&&!queued)return;replaceOperation({key:appointmentId,action:"delete",appointmentId})})}
export async function queueAllGoogleAppointments(appointments:Appointment[],patients:Patient[]){
 if(!getGoogleCalendarPreferences().enabled)return;
 await withGoogleQueueMutationLock(async()=>{
...
async function flushPass(){
 await withGoogleFlushLock(async()=>{
 ...
 try{
  ...
  const errors=await processQueueSnapshot(initial,id=>read<Operation[]>(QUEUE_KEY,[]).find(item=>item.id===id),async operation=>{
   ...
   if(operation.action==="delete"&&!cloudMode&&!mapping[operation.appointmentId]){
    await withGoogleQueueMutationLock(async()=>write(QUEUE_KEY,read<Operation[]>(QUEUE_KEY,[]).filter(item=>item.id!==operation.id)));
    announce();
    completed=true;
    return;
   }
...
const flushRunner=createSingleFlightRunner(flushPass);
export async function flushGoogleCalendarQueue(){if(!getGoogleCalendarPreferences().enabled)return;const result=flushRunner.request();announce();await result;announce()}
