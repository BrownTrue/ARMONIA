...
import { addClinicalModule, clinicalAssessmentTypeLabel, createAssessmentTestV2, createClinicalAssessmentV2, createConfiguredClinicalAssessmentV2, INITIAL_ASSESSMENT_STEP, isClinicalAssessmentV2ReadOnly, isClinicalModuleEmpty, removeClinicalModule, requiresClinicalModuleRemovalConfirmation, setClinicalAssessmentV2Section, shouldAutosaveClinicalAssessmentV2 } from "./assessment-v2.ts";
import { autosaveClinicalAssessmentDraft, completeClinicalAssessment, correctClinicalAssessment, createClinicalAssessmentDraft, deleteClinicalAssessment, deleteClinicalPathway, getClinicalAssessment } from "./local.ts";
import { clinicalModuleRegistry, createClinicalModuleInstance, getClinicalModuleDefinition, isAacMultimodalModuleV1, isAuditoryCommunicationModuleV1, isCognitiveCommunicationModuleV1, isFeedingSwallowingModuleV1, isFluencyModuleV1, isLanguageOralModuleV1, isMotorSpeechModuleV1, isOrofacialFunctionsModuleV1, isSchoolLearningModuleV1, isSocialPragmaticsModuleV1, isSpeechSoundModuleV1, isVoiceModuleV1, toClinicalAssessmentV2PrintSections } from "./module-registry.ts";
...
test("il registry espone i dodici moduli nell'ordine UI previsto", () => {
  assert.deepEqual(clinicalModuleRegistry.map(({ code, version, label }) => ({ code, version, label })), [
    ...
    { code: "school_learning", version: 1, label: "Apprendimenti scolastici" },
    { code: "cognitive_communication", version: 1, label: "Funzioni cognitivo-comunicative" },
    { code: "social_pragmatics", version: 1, label: "Comunicazione sociale e pragmatica" },
  ]);
});
...
test("un assessment con tutti i moduli è valido e non duplica le aree", () => {
  ...
  assert.equal(assessment.data.modules.length, 12);
  assert.equal(isClinicalAssessment(assessment), true);
...
test("apprendimenti scolastici valida solo osservazioni funzionali previste", () => {
  assert.equal(isSchoolLearningModuleV1({}), true);
  assert.equal(isSchoolLearningModuleV1({ profile: { status: "further_assessment" } }), true);
  assert.equal(isSchoolLearningModuleV1({ profile: { status: "difficulties_observed", contextsExplored: ["reading_aloud", "dictation"], exploredAreas: ["reading_fluency", "spelling"], observedFeatures: ["slow_or_effortful_reading", "spelling_errors"], notes: "Osservazione funzionale." } }), true);
  assert.equal(isSchoolLearningModuleV1({ profile: { status: "difficulties_observed", observedFeatures: ["dyslexia"] } }), false);
  assert.equal(isSchoolLearningModuleV1({ profile: { status: "difficulties_observed", diagnosticScore: 12 } }), false);
});

test("funzioni cognitivo-comunicative esclude campi neuropsicologici generali", () => {
  assert.equal(isCognitiveCommunicationModuleV1({}), true);
  assert.equal(isCognitiveCommunicationModuleV1({ profile: { status: "no_evident_difficulty" } }), true);
  assert.equal(isCognitiveCommunicationModuleV1({ profile: { status: "difficulties_observed", contextsExplored: ["multistep_activity"], exploredAreas: ["verbal_working_memory", "message_planning"], observedFeatures: ["difficulty_following_multistep_information", "message_planning_difficulty"], notes: "Impatto comunicativo osservato." } }), true);
  assert.equal(isCognitiveCommunicationModuleV1({ profile: { status: "difficulties_observed", exploredAreas: ["nonverbal_iq"] } }), false);
  assert.equal(isCognitiveCommunicationModuleV1({ profile: { status: "difficulties_observed", dementiaScreening: true } }), false);
});

test("comunicazione sociale e pragmatica valida osservazioni senza diagnosi", () => {
  assert.equal(isSocialPragmaticsModuleV1({}), true);
  assert.equal(isSocialPragmaticsModuleV1({ profile: { status: "further_assessment" } }), true);
  assert.equal(isSocialPragmaticsModuleV1({ profile: { status: "difficulties_observed", contextsExplored: ["structured_interaction"], exploredAreas: ["turn_taking", "inferential_language"], observedFeatures: ["turn_taking_difficulty", "inferential_difficulty"], notes: "Osservazione contestuale." } }), true);
  assert.equal(isSocialPragmaticsModuleV1({ profile: { status: "difficulties_observed", observedFeatures: ["autism"] } }), false);
  assert.equal(isSocialPragmaticsModuleV1({ profile: { status: "difficulties_observed", diagnosis: "pragmatic_disorder" } }), false);
});

test("gli adapter dei tre nuovi moduli omettono il vuoto e producono label leggibili", () => {
  for (const code of ["school_learning", "cognitive_communication", "social_pragmatics"]) assert.deepEqual(getClinicalModuleDefinition(code, 1)?.toPrintSections({}), []);
  const school = getClinicalModuleDefinition("school_learning", 1)?.toPrintSections({ profile: { status: "difficulties_observed", exploredAreas: ["reading_fluency"] } }) || [];
  assert.deepEqual(school[0]?.fields[1]?.value, ["Fluenza di lettura"]);
  const cognitive = getClinicalModuleDefinition("cognitive_communication", 1)?.toPrintSections({ profile: { status: "further_assessment", observedFeatures: ["loses_track_of_conversation"] } }) || [];
  assert.deepEqual(cognitive[0]?.fields[1]?.value, ["Perdita del filo della conversazione"]);
  const social = getClinicalModuleDefinition("social_pragmatics", 1)?.toPrintSections({ profile: { status: "difficulties_observed", exploredAreas: ["turn_taking", "inferential_language"] } }) || [];
  assert.deepEqual(social[0]?.fields[1]?.value, ["Gestione dei turni", "Comprensione inferenziale"]);
  const ordered = toClinicalAssessmentV2PrintSections({ modules: [
    { code: "social_pragmatics", version: 1, data: { profile: { status: "further_assessment" } } },
    { code: "school_learning", version: 1, data: { profile: { status: "difficulties_observed" } } },
    { code: "cognitive_communication", version: 1, data: {} },
  ] });
  assert.deepEqual(ordered.map((section) => section.code), ["social_pragmatics.profile", "school_learning.profile"]);
});
...
test("draft e completed V2 sono eliminabili senza cancellare percorso o altre entità", () => {
  const assessment = createConfiguredClinicalAssessmentV2({ patientId: "patient-1", clinicalPathwayId: "pathway-1", assessmentType: "initial", clinicalDate: "2026-10-05", modules: [{ code: "language_oral", version: 1 }] });
  const base = { patients: [{ id: "patient-1" }], appointments: [{ id: "appointment-1" }], sessions: [{ id: "session-1" }], goals: [{ id: "goal-1" }], materials: [{ id: "material-1" }], clinicalPathways: [{ id: "pathway-1", patientId: "patient-1", status: "active", startedOn: "2026-10-05", createdAt: assessment.createdAt, updatedAt: assessment.updatedAt }], clinicalAssessments: [], profile: { firstName: "", lastName: "", profession: "", email: "", studio: "" } };
  const draftData = createClinicalAssessmentDraft(base, assessment);
  assert.throws(() => deleteClinicalPathway(draftData, "pathway-1"), /contiene valutazioni/);
  const withoutDraft = deleteClinicalAssessment(draftData, assessment.id);
  assert.equal(withoutDraft.clinicalAssessments.length, 0);
  assert.equal(withoutDraft.clinicalPathways.length, 1);
  assert.deepEqual(withoutDraft.patients, base.patients);
  assert.deepEqual(withoutDraft.appointments, base.appointments);
  assert.deepEqual(withoutDraft.sessions, base.sessions);
  assert.deepEqual(withoutDraft.goals, base.goals);
  assert.deepEqual(withoutDraft.materials, base.materials);
  assert.equal(deleteClinicalPathway(withoutDraft, "pathway-1").clinicalPathways.length, 0);

  const completedData = completeClinicalAssessment(createClinicalAssessmentDraft(base, assessment), assessment.id);
  assert.equal(isClinicalAssessmentV2ReadOnly(getClinicalAssessment(completedData, assessment.id)), true);
  const withoutCompleted = deleteClinicalAssessment(completedData, assessment.id);
  assert.equal(withoutCompleted.clinicalAssessments.length, 0);
  assert.equal(withoutCompleted.clinicalPathways.length, 1);
  assert.deepEqual(withoutCompleted.sessions, base.sessions);
  assert.deepEqual(withoutCompleted.goals, base.goals);
});
...
test("registry contiene i dodici moduli v1 e le factory producono payload vuoti", () => {
  assert.deepEqual(clinicalModuleRegistry.map(({ code, version, label }) => ({ code, version, label })), [
    { code: "language_oral", version: 1, label: "Linguaggio orale" },
    { code: "speech_sound", version: 1, label: "Fonetica, fonologia e articolazione" },
    { code: "motor_speech", version: 1, label: "Disturbi motori del parlato" },
...
test("creazione supporta un solo modulo o più moduli con payload iniziali vuoti", () => {
  ...
  assert.equal(multiple.data.modules.length, 12);
...
test("le macro-aree UI coprono una sola volta tutti i moduli nell'ordine previsto", () => {
  ...
  assert.deepEqual(clinicalModuleGroups.map(({ code, label, order }) => ({ code, label, order })), [
    ...
    { code: "learning_cognitive_functions", label: "Apprendimenti e funzioni cognitive", order: 5 },
  ]);
  assert.deepEqual(clinicalModuleGroups.map((group) => getClinicalModulesForGroup(group).map((module) => module.code)), [
    ["language_oral", "speech_sound", "motor_speech", "fluency", "aac_multimodal", "social_pragmatics"],
    ["voice"],
    ["feeding_swallowing", "orofacial_functions"],
    ["auditory_communication"],
    ["school_learning", "cognitive_communication"],
  ]);
});
...
test("autosave, completamento e correzione conservano una assessment multi-modulo", () => {
  ...
  const created = createClinicalAssessmentDraft(data, assessment);
  const changed = { ...assessment, data: { modules: assessment.data.modules.map((module) => module.code === "school_learning" ? { ...module, data: { profile: { status: "further_assessment", exploredAreas: ["reading_fluency"] } } } : module) } };
  const saved = autosaveClinicalAssessmentDraft(created, changed);
  const completed = completeClinicalAssessment(saved, assessment.id);
  const correctedAssessment = { ...getClinicalAssessment(completed, assessment.id), data: { ...getClinicalAssessment(completed, assessment.id).data, modules: getClinicalAssessment(completed, assessment.id).data.modules.map((module) => module.code === "social_pragmatics" ? { ...module, data: { profile: { status: "no_evident_difficulty" } } } : module) } };
  ...
  assert.equal(getClinicalAssessment(corrected, assessment.id).data.modules.find((module) => module.code === "school_learning").data.profile.exploredAreas[0], "reading_fluency");
});
