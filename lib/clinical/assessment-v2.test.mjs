...
import { clinicalModuleRegistry, createClinicalModuleInstance, getClinicalModuleDefinition, isFeedingSwallowingModuleV1, isFluencyModuleV1, isLanguageOralModuleV1, isMotorSpeechModuleV1, isOrofacialFunctionsModuleV1, isSpeechSoundModuleV1, isVoiceModuleV1, toClinicalAssessmentV2PrintSections } from "./module-registry.ts";
...
test("il registry espone i sette moduli nell'ordine UI previsto", () => {
...
test("un assessment con tutti i moduli è valido e non duplica le aree", () => {
  ...
  assert.equal(assessment.data.modules.length, 7);
  assert.equal(isClinicalAssessment(assessment), true);
  ...
  assert.throws(() => removeClinicalModule({ ...assessment, data: { ...assessment.data, modules: [assessment.data.modules[0]] } }, "language_oral", 1), /almeno un.area clinica/);
});
...
test("registry contiene i sette moduli v1 e le factory producono payload vuoti", () => {
  assert.deepEqual(clinicalModuleRegistry.map(({ code, version, label }) => ({ code, version, label })), [
    { code: "language_oral", version: 1, label: "Linguaggio orale" },
    { code: "speech_sound", version: 1, label: "Fonetica, fonologia e articolazione" },
    { code: "motor_speech", version: 1, label: "Disturbi motori del parlato" },
...
test("creazione supporta un solo modulo o più moduli con payload iniziali vuoti", () => {
  ...
  assert.equal(multiple.data.modules.length, 7);
