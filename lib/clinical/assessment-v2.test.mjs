...
import { clinicalModuleRegistry, createClinicalModuleInstance, getClinicalModuleDefinition, isFluencyModuleV1, isLanguageOralModuleV1, isSpeechSoundModuleV1, isVoiceModuleV1, toClinicalAssessmentV2PrintSections } from "./module-registry.ts";
...
test("la validazione V2 rifiuta moduli sconosciuti senza indebolire la V1", () => {
  const assessment = createClinicalAssessmentV2("patient-1", "pathway-1");
  assessment.data.modules.push({ code: "motor_speech", version: 1, data: {} });
...
test("registry contiene i quattro moduli v1 e le factory producono payload vuoti", () => {
...
test("creazione supporta un solo modulo o più moduli con payload iniziali vuoti", () => {
  ...
  assert.equal(multiple.data.modules.length, 4);
...
test("gestione aree aggiunge senza duplicare e distingue moduli vuoti o compilati", () => {
  ...
  assert.throws(() => removeClinicalModule(removeClinicalModule(assessment, "fluency", 1), "language_oral", 1), /almeno un.area/);
});
...
test("autosave, completamento e correzione conservano una assessment multi-modulo", () => {
  const assessment = createConfiguredClinicalAssessmentV2({ patientId: "patient-1", clinicalPathwayId: "pathway-1", assessmentType: "initial", clinicalDate: "2026-10-06", modules: [{ code: "speech_sound", version: 1 }, { code: "fluency", version: 1 }, { code: "voice", version: 1 }] });
  ...
  const created = createClinicalAssessmentDraft(data, assessment);
  const changed = { ...assessment, data: { modules: assessment.data.modules.map((module) => module.code === "voice" ? { ...module, data: { profile: { status: "further_assessment" } } } : module) } };
  ...
  const correctedAssessment = { ...getClinicalAssessment(completed, assessment.id), data: { ...getClinicalAssessment(completed, assessment.id).data, modules: getClinicalAssessment(completed, assessment.id).data.modules.map((module) => module.code === "fluency" ? { ...module, data: { profile: { status: "no_evident_difficulty" } } } : module) } };
  const corrected = correctClinicalAssessment(completed, correctedAssessment);
  assert.equal(getClinicalAssessment(corrected, assessment.id).status, "completed");
  assert.deepEqual(getClinicalAssessment(corrected, assessment.id).data.modules.map((module) => module.code), ["speech_sound", "fluency", "voice"]);
});
