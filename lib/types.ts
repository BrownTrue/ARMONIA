...
export type Appointment = { id:string; patientId:string; date:string; time:string; duration:number; type:AppointmentType; notes:string; recurrenceSeriesId?:string; createdAt:string };
export type Session = { id:string; patientId:string; appointmentId?:string; date:string; duration:number; goalIds:string[]; activities:string; response:string; helpLevel:string; result:string; nextPlan:string; homework:string; notes:string; materialIds:string[]; createdAt:string };
